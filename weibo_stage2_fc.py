...
class MMRumor(object):
    ...
    def __init__(self, root='data', **kwargs):
        ...
        print("  ------------------------------")
        print("  total    | {:5d}   | {:8d}".format(len(train_rumor)+len(test_rumor), len(train_nonrumor)+len(test_nonrumor)))
        print("  ------------------------------")
...
def main():
    ...
    for epoch in range(1, args.epochs_num + 1):
        ...
        if acc > best_acc:
            ...
            save_file = os.path.join(config.save_folder, 'fc_crd_mutual.pth'.format(args.crd_op))
    ...
    #             'best_acc': best_acc,
    #         }
    #         save_file = os.path.join(args.save_folder, 'fc.pth')
    #         print('saving  for fc!')
    #         torch.save(state, save_file)
    #
    #
...
def validate_2_stage(val_loader, module_list, criterion):
    ...
    #     print('val * F1{F1.avg:.3f} '.format(F1=F1_lists))
    #
